...
def summarize(trials, key_fn):
    ...
    for k, ts in groups.items():
        ...
        unfaithful = sum(1 for t in ts if t.get("flip_type") == "unfaithful")
        faithful = sum(1 for t in ts if t.get("flip_type") == "faithful")
        held = sum(1 for t in ts if t.get("flip_type") == "held_firm")
        ...
        out[k] = {
            ...
            "faithful_flips": faithful,
            "unfaithful_flips": unfaithful,
            "held_firm": held,
            "first_answer_accuracy": sum(1 for t in ts if t["first_correct"]) / n,
            "final_answer_accuracy": sum(1 for t in ts if t["final_correct"]) / n,
        }
    return out
...
def main():
    ...
    by_condition = summarize(trials, lambda t: t["condition"])
    by_model = summarize(trials, lambda t: t["model"])
    by_condition_model = summarize(trials, lambda t: f"{t['model']}__{t['condition']}")
    ...
    results = {
        ...
        "by_model": by_model,
        "by_condition_and_model": by_condition_model,
        "trials": trials,
    }
    ...
    for k, v in by_condition.items():
        print(f"  {k}: {v['flips']}/{v['n']} flips, "
              f"95% CI upper bound {v['flip_rate_ci95'][1]*100:.1f}%")
